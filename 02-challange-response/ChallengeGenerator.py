...
def generateChallenge(username, entries=Global.DEFAULT_KEY_NUMBER, output_folder="users", expiration_date=None):
    ...
    keys = [generateKeyword(alphabet, Global.KEY_LENGTH) for k in range(entries)]
    challenges = [generateKeyword(alphabet, Global.CHALLENGE_LENGTH) for k in range(entries)]
